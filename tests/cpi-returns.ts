...
describe("cpi-returns", () => {
  ...
  const callerProgram = anchor.workspace.Caller;
  const calleeProgram = anchor.workspace.Callee;
  ...
  it("can return u64 from a cpi", async () => {
    ...
    assert.equal(key, calleeProgram.programId.toBase58());
  ...
  it("can return a vec via view", async () => {
    ...
    assert(vec[3].eq(new anchor.BN(4)));
    ...
    assert(vec2[3].eq(new anchor.BN(4)));
  });
});
